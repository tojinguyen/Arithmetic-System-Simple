...
def test_calculate_valid_expressions(
    client: TestClient, expression: str, expected_result: float
):
...
def test_calculate_invalid_expressions(
    client: TestClient, expression: str, error_message_part: str
):
    response = client.get("/api/calculate", params={"expression": expression})

    assert response.status_code == 400, (
        f"Expected 400 on expression: '{expression}', but got {response.status_code}"
    )
